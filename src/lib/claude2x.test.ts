...
describe("getStatus", () => {
	...
	describe("promo boundaries", () => {
		...
		it("just before promo end", () => {
			expect(getStatus(dayjs.utc("2026-03-28 06:59:59")).promoActive).toBe(true)
		})

		it("just after promo end", () => {
			expect(getStatus(dayjs.utc("2026-03-28 07:00:01")).promoEnded).toBe(true)
		})
	})
	...
	describe("promo end boundary (end of Friday March 27)", () => {
		...
		it("last 2x of promo (March 27 11:59 PM PDT = 06:59 UTC March 28)", () => {
			const s = getStatus(dayjs.utc("2026-03-28 06:59:59"))
			expect(s.promoActive).toBe(true)
			expect(s.is2x).toBe(true)
		})

		it("promo over at midnight PDT March 28 (07:00 UTC March 28)", () => {
			const s = getStatus(dayjs.utc("2026-03-28 07:00:00"))
...
describe("getCountdown", () => {
	...
	describe("promo boundary countdowns", () => {
		...
		it("caps at promo end on last day", () => {
			...
			// but promo ends March 28, 07:00 UTC
	...
	describe("last day of promo edge cases", () => {
		...
		it("Friday March 27, after peak → promo ends (next peak would be after promo)", () => {
			...
			const cd = getCountdown(now)
			expect(cd.label).toBe("Promotion ends")
			// Promo ends at March 28 07:00 UTC = 13h
			expect(cd.seconds).toBe(13 * 3600)
		})

		it("March 28, 06:59:59 UTC (last second of promo, 11:59:59 PM PDT)", () => {
			const now = dayjs.utc("2026-03-28 06:59:59")
			const cd = getCountdown(now)
			expect(cd.label).toBe("Promotion ends")
			expect(cd.seconds).toBe(1)
		})
...
describe("constants", () => {
	...
	it("PROMO_END last valid moment is Friday March 27 (11:59:59 PM PDT)", () => {
		// PROMO_END = March 28 07:00 UTC = March 28 00:00 PDT (midnight)
		// Last valid moment is 1 second before = March 27 11:59:59 PM PDT
